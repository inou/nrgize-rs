...
fn deploy_app_finds_the_named_container_and_patches_its_image_with_the_access_key_header() {
    ...
    let get_req = rx
        ...
        .expect("no GET received");
    assert!(get_req.contains("GET /mc/apps/app1"), "{get_req}");
    assert!(
        get_req.to_lowercase().contains("accesskey: testkey"),
        "{get_req}"
    );

    let patch_req = rx
        .recv_timeout(std::time::Duration::from_secs(5))
        .expect("no PATCH received");
    assert!(
    ...
    assert!(
        patch_req.to_lowercase().contains("accesskey: testkey"),
        "{patch_req}"
    );
    assert!(
...
fn deploy_app_skips_the_snapshot_when_the_container_reports_no_image_tag() {
    ...
    assert!(
        ok,
        "deploy_app must still succeed even with no reported current tag:\n{out}"
    );
    assert!(
...
fn rollback_app_under_dry_run_still_makes_a_real_get_but_never_a_real_patch() {
    ...
    assert!(ok2, "{out2}");
    assert!(
        out2.contains("STATUS=200"),
        "PATCH must synthesize a 200 under dry-run:\n{out2}"
    );
    assert!(
    ...
    let _get = rx2
        .recv_timeout(std::time::Duration::from_secs(5))
        .expect("the GET must be real");
    assert!(
        rx2.recv_timeout(std::time::Duration::from_millis(300))
            .is_err(),
...
fn api_key_accepted_as_a_bare_secret_and_revealed_before_use() {
    ...
    assert!(
        get_req
            .to_lowercase()
            .contains("accesskey: real-secret-key"),
...
fn ok_target_responder(
    ...
    spawn_bunny_responder(vec![
        Box::leak(
            app_config_response(&format!(
                r#"{{"id":"c-web","name":"web","imageTag":"{old_tag}"}}"#
            ))
            .into_boxed_str(),
        ),
        patch_ok_response(),
        Box::leak(
            app_config_response(&format!(
                r#"{{"id":"c-web","name":"web","imageTag":"{new_tag}"}}"#
            ))
            .into_boxed_str(),
        ),
    ])
...
fn deploy_fleet_aborts_during_canary_before_touching_the_rest_of_the_fleet() {
    ...
    assert!(
        !ok,
        "must throw once the canary failure exceeds max_failures: 0:\n{out}"
    );
    assert!(
        out.contains("app-a"),
        "error must name the failed target:\n{out}"
    );
    assert!(
        rx_b.recv_timeout(std::time::Duration::from_millis(300))
            .is_err(),
...
fn deploy_fleet_carries_a_canary_failure_under_threshold_into_the_batch_phase_count() {
    ...
    assert!(
        out.contains("app-a") && out.contains("app-b"),
...
fn deploy_fleet_batch_phase_snapshots_each_targets_previous_tag_for_rollback() {
    ...
    let (ok2, out2) = run_in(
        r#"print("PREV=" + state_get("bunny.app-a.web.prev"));"#,
        &dir,
        false,
    );
    assert!(ok2, "{out2}");
...
fn deploy_fleet_canary_phase_also_counts_a_health_url_failure_as_a_target_failure() {
    ...
    assert!(
        ok,
        "one canary health failure within max_failures: 1 must not throw:\n{out}"
    );
    assert!(
...
fn deploy_fleet_tolerates_a_batch_failure_within_the_threshold_and_reports_it() {
    ...
    assert!(
        ok,
        "one failure within max_failures: 1 must not throw:\n{out}"
    );
    assert!(
        out.contains("SUCCEEDED=1 FAILED=1 FIRST_FAIL=app-bad"),
        "{out}"
    );
}
...
fn deploy_fleet_throws_once_failures_exceed_the_threshold_and_stops_dispatching_further_batches() {
    ...
    assert!(
        !ok,
        "must throw once the first batch's failure exceeds max_failures: 0:\n{out}"
    );
    assert!(
        out.contains("app-a"),
        "error must name the failed target:\n{out}"
    ...
    assert!(
        rx_c.recv_timeout(std::time::Duration::from_millis(300))
            .is_err(),
...
fn deploy_fleet_under_dry_run_never_makes_a_real_patch_for_any_target() {
    ...
    let _get_a = rx_a
        .recv_timeout(std::time::Duration::from_secs(5))
        .expect("GET must be real");
    assert!(
        rx_a.recv_timeout(std::time::Duration::from_millis(300))
            .is_err(),
        "no real PATCH for app-a under --dry-run"
    );
    let _get_b = rx_b
        .recv_timeout(std::time::Duration::from_secs(5))
        .expect("GET must be real");
    assert!(
        rx_b.recv_timeout(std::time::Duration::from_millis(300))
            .is_err(),
        "no real PATCH for app-b under --dry-run"
    );
}
...
fn deploy_fleet_counts_a_health_url_failure_as_a_target_failure_even_though_the_patch_succeeded() {
    ...
    assert!(
        ok,
        "one health failure within max_failures: 1 must not throw:\n{out}"
    );
    assert!(
        out.contains("SUCCEEDED=0 FAILED=1"),
...
fn deploy_app_refuses_plausible_near_miss_keys_regions_and_replica() {
    ...
    for key in ["regions", "replica"] {
        ...
        assert!(
            out.contains(key),
            "error must name the offending key \"{key}\":\n{out}"
        );
        assert!(
            rx.recv_timeout(std::time::Duration::from_millis(300))
                .is_err(),
...
fn deploy_fleet_refuses_a_zone_key_on_the_shared_cfg_before_contacting_anything() {
    ...
    assert!(
        !ok,
        "must refuse a cfg.zone key on the shared fleet cfg:\n{out}"
    );
    assert!(
        out.contains("zone"),
        "error must name the offending key:\n{out}"
...
fn create_app_requires_every_mandatory_key_before_contacting_anything() {
    for (skip, _) in CREATE_APP_MANDATORY_FIELDS {
        ...
        assert!(
            out.contains(skip),
            "error must name the missing key \"{skip}\":\n{out}"
        );
        assert!(
            rx.recv_timeout(std::time::Duration::from_millis(300))
                .is_err(),
...
fn create_app_accepts_an_env_entry_with_an_explicitly_empty_value() {
    ...
    assert!(
        ok,
        "an env entry with an explicitly empty value must be accepted:\n{out}"
    );
    let req = rx.recv_timeout(std::time::Duration::from_secs(5)).unwrap();
    assert!(
        req.contains("\"name\":\"FOO\"") && req.contains("\"value\":\"\""),
        "{req}"
    );
}
...
fn create_app_region_id_does_not_collide_with_the_region_denylist_entry() {
    ...
    assert!(
        ok,
        "a cfg using the correct region_id key must not be refused:\n{out}"
    );
    assert!(out.contains("ID=new-app"), "{out}");

    let req = rx
        .recv_timeout(std::time::Duration::from_secs(5))
        .expect("no POST received");
...
fn create_app_posts_the_expected_body_and_returns_the_new_app() {
    ...
    assert!(ok, "{out}");
    assert!(
        out.contains("ID=new-app"),
        "must return the created app (from_json(r.body)):\n{out}"
    );
...
fn create_app_under_dry_run_never_makes_a_real_post() {
    ...
    assert!(
        ok,
        "create_app must not crash trying to parse an empty dry-run body:\n{out}"
    );
    assert!(out.contains("DONE"), "{out}");
    assert!(
...
fn delete_app_refuses_a_missing_or_empty_app_id_before_contacting_anything() {
    ...
    for cfg_body in [
        ...
        assert!(
            !ok,
            "must refuse a missing/empty app_id ({cfg_body}):\n{out}"
        );
        assert!(
            out.contains("app_id"),
...
fn delete_app_under_dry_run_never_makes_a_real_delete() {
    ...
    assert!(
        out.contains("STATUS=200"),
        "DELETE must synthesize a 200 under dry-run:\n{out}"
    );
    assert!(
...
#[test]
fn audit_rollback_restores_full_previous_image_identity() {
    let (addr, rx) = spawn_bunny_responder(vec![
        Box::leak(app_config_response(r#"{"id":"c-web","name":"web","imageTag":"v1","imageName":"old/image","imageDigest":"sha256:old"}"#).into_boxed_str()),
        patch_ok_response(),
        Box::leak(app_config_response(r#"{"id":"c-web","name":"web","imageTag":"v2","imageName":"new/image","imageDigest":"sha256:new"}"#).into_boxed_str()),
        patch_ok_response(),
    ]);
    let (ok, out) = run_live(&format!(
        r#"
        import "std/bunny" as b;
        let cfg = #{{app_id: "app1", container: "web", image_tag: "v2",
            image_name: "new/image", image_digest: "sha256:new", api_key: "dummy-key", base_url: "http://{addr}"}};
        b::deploy_app(cfg);
        b::rollback_app(cfg);
    "#
    ));
    ...
    let requests: Vec<_> = (0..4)
        ...
        .collect();
    assert!(
        requests[3].contains(r#""imageName":"old/image""#),
        "{}",
        requests[3]
    );
    assert!(
        requests[3].contains(r#""imageDigest":"sha256:old""#),
        "{}",
        requests[3]
    );
}
...
#[test]
fn audit_noop_bunny_deploy_preserves_predecessor() {
    let (addr, _rx) = spawn_bunny_responder(vec![
        Box::leak(
            app_config_response(r#"{"id":"c-web","name":"web","imageTag":"v2"}"#).into_boxed_str(),
        ),
    ...
    let (ok, out) = run_live(&format!(
        r#"
        import "std/bunny" as b;
        state_set("bunny.app1.web.prev", "v1");
        b::deploy_app(#{{app_id:"app1", container:"web", image_tag:"v2", api_key:"dummy-key", base_url:"http://{addr}"}});
        if state_get("bunny.app1.web.prev") != "v1" {{ throw "predecessor overwritten"; }}
    "#
    ));
    assert!(ok, "{out}");
}
...
#[test]
fn audit_revision_gate_rejects_health_from_the_previous_version() {
    let (addr, _rx) = spawn_bunny_responder(vec![
        Box::leak(
            app_config_response(r#"{"id":"c-web","name":"web","imageTag":"v1"}"#).into_boxed_str(),
        ),
    ...
    let (health, _health_rx) = spawn_bunny_responder(vec![
        "HTTP/1.1 200 OK\r\nConnection: close\r\nContent-Length: 2\r\n\r\nv1",
    ]);
    let (ok, out) = run_live(&format!(
        r#"
        import "std/bunny" as b;
        state_set("bunny.app1.web.prev", "v0");
        let result = b::deploy_fleet([#{{app_id:"app1", container:"web", image_tag:"v2", base_url:"http://{addr}", health_url:"http://{health}"}}],
            #{{api_key:"dummy-key", require_version_health:true, health_attempts:1, health_interval:0, max_failures:1}});
        if result.failed.len() != 1 {{ throw "old revision was accepted"; }}
        if state_get("bunny.app1.web.prev") != "v0" {{ throw "failed canary overwrote history"; }}
    "#
    ));
    assert!(ok, "{out}");
}
