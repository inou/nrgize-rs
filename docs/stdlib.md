...
| `platform` | `""` | A single target platform (e.g. `"linux/amd64"`) other than the build machine's own. When set, uses `buildx build --platform <value> --load` instead of a plain `build` — needed when building on, say, an Apple Silicon laptop for an x86 VPS. `--load` keeps the result a normal local image, so the separate `docker_push` step still works. Not a multi-platform manifest list. |
